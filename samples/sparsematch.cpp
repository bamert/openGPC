...
int main(int argc, char** argv) {
  ...
  gpc::inference::InferenceSettings inferencesettings = gpc::inference::InferenceSettings().builder()
    .gradientThreshold(5)  
    .verticalTolerance(0) // 0px tolerance for rectified epipolar matches
    .dispHigh(128)        // limit disparities to 128
    .epipolarMode(true)   // match GPC states in epipolar mode. more matches, lower accuracy than global
  ...
  GPCForest_t::PreprocessedImage simgP = forest.preprocessImage(simg, inferencesettings);
  GPCForest_t::PreprocessedImage timgP = forest.preprocessImage(timg, inferencesettings);
